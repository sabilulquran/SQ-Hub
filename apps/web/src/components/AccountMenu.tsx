      ...
      >
        <span
          className={[
            ...
            header ? "h-8 w-8 rounded-xl text-xs" : "h-11 w-11 rounded-2xl text-sm",
          ].join(" ")}
        >
          {user.initials}
        </span>
        <span className={header ? "hidden min-w-0 max-w-40 sm:block" : "min-w-0"}>
          <span className={header ? "block truncate text-xs font-bold" : "block truncate text-sm font-bold"}>
            {user.displayName}
          </span>
          <span className={header ? "block truncate text-[10px] text-muted-foreground" : "block truncate text-[11px] text-muted-foreground"}>
      ...
      {open ? (
        <div
          ...
          className={[
            ...
            header ? "right-0 top-full mt-2" : "bottom-full left-0 mb-2",
          ].join(" ")}
        >
          ...
          <div className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-muted-foreground" aria-hidden="true">
            ...
            <span className="text-[11px] leading-4">Keamanan akun dikelola oleh SQ Identity</span>
          </div>
          ...
          <button
            ...
            className="mt-1 flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-semibold text-destructive transition-colors hover:bg-destructive/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-45"
          >
