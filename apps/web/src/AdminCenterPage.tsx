...
export function AdminCenterPage({
  ...
  const saveApplication = async () => {
    ...
    try {
      ...
      setState((current) => {
        ...
        const exists = current.applications.some(
          (application) => application.key === body.application.key,
        );
        return {
          status: "ready",
          applications: exists
  ...
  const mutateAccess = async (applicationKey: string, action: "grant" | "revoke") => {
    ...
    if (
      action === "revoke" &&
      !window.confirm(`Cabut akses ${selectedStaff.displayName} ke ${applicationKey}?`)
    ) {
      return;
    }
    setAccessLoading(true);
      ...
      <div className="lg:pl-72">
        ...
        <main className="mx-auto max-w-7xl px-4 pb-28 pt-6 sm:px-6 sm:pt-8 lg:px-8 lg:pb-12">
          <div className="mb-6 lg:hidden">
            <a
              href="/"
              className="inline-flex items-center gap-2 text-sm font-semibold text-brand-primary-deep"
            >
          ...
          <section className="rounded-[2rem] border border-brand-primary/15 bg-white p-6 shadow-[var(--shadow-soft)] sm:p-8">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
              <div className="max-w-3xl">
                <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-brand-primary-deep">
                  SQ Admin Center · Go 5B
                </p>
                <h2 className="mt-2 font-display text-3xl font-bold tracking-[-0.03em] text-brand-heading sm:text-4xl">
                  Administrasi SQ
                </h2>
                <p className="mt-3 text-sm leading-6 text-muted-foreground sm:text-base sm:leading-7">
                  Kelola registry dan pintu masuk aplikasi lintas platform. Hak payroll, kepegawaian,
                  keuangan, penerimaan, dan kewenangan bisnis tetap berada di aplikasi masing-masing.
                </p>
              </div>
          ...
          {state.status === "loading" ? (
            <LoadingCard label="Memeriksa kewenangan administrasi..." />
          ) : null}
          {state.status === "forbidden" ? (
            <AdminMessage
          ...
          {state.status === "reauth" ? (
            <AdminMessage
              ...
              body="Kewenangan administrator diberikan setelah sesi ini dibuat. Keluar lalu masuk kembali melalui SQ Identity."
          ...
          {state.status === "ready" ? (
            <>
              <div
                ...
                role="tablist"
                aria-label="Bagian Administrasi SQ"
              >
                <SectionButton
                ...
                <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(20rem,0.8fr)]">
                  <div>
                    <SectionHeading
                      ...
                      description="Kunci aplikasi stabil dan tidak dapat diganti setelah dibuat. Status nonaktif menutup akses baru tanpa memindahkan hak domain ke SQ Hub."
                    />
                    <div className="mt-5 grid gap-4 md:grid-cols-2">
                      {applications.map((application) => (
                        <button
                          key={application.key}
                          type="button"
                          onClick={() => setApplicationForm({ ...application })}
                          className="rounded-3xl border border-border/70 bg-white p-5 text-left shadow-[var(--shadow-soft)] transition hover:-translate-y-0.5 hover:border-brand-primary/35"
                        >
                          <div className="flex items-start justify-between gap-4">
                            <div className="min-w-0">
                              <h3 className="truncate font-display text-lg font-bold text-brand-heading">
                                {application.name}
                              </h3>
                  ...
                  <div className="rounded-3xl border border-border/70 bg-white p-5 shadow-[var(--shadow-soft)]">
                    <div className="flex items-center justify-between gap-3">
                      <h3 className="font-display text-xl font-bold text-brand-heading">
                        {editingExisting ? "Ubah aplikasi" : "Daftarkan aplikasi"}
                      </h3>
                      <button
                        type="button"
                        onClick={() => {
                        ...
                        className="inline-flex items-center gap-1.5 rounded-xl border border-border px-3 py-2 text-xs font-bold text-brand-primary-deep"
                      >
                        <Plus className="h-3.5 w-3.5" /> Baru
                      </button>
                    </div>
                    <div className="mt-5 space-y-4">
                      <Field label="Kunci aplikasi">
                        <input
                      ...
                      <Field label="URL kanonis">
                        <input
                          value={applicationForm.canonicalUrl}
                          onChange={(event) =>
                            setApplicationForm((current) => ({
                              ...current,
                    ...
                    <button
                      type="button"
                      disabled={applicationSaving || Boolean(previewApplications)}
                      onClick={() => void saveApplication()}
                      className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-brand-primary px-4 py-3 text-sm font-bold text-white shadow-[var(--shadow-button)] disabled:cursor-not-allowed disabled:opacity-50"
                    >
                      <Save className="h-4 w-4" />
                      {applicationSaving ? "Menyimpan..." : "Simpan registry"}
                    </button>
                  </div>
                </section>
              ) : null}
              ...
              {section === "access" ? (
                <section className="mt-6">
                  <SectionHeading
                    ...
                    title="Akses aplikasi Staff"
                    description="Cari Staff, periksa kesiapan MFA, lalu berikan atau cabut pintu masuk aplikasi. Ini tidak mengubah role atau permission di aplikasi domain."
                  />
                  <div className="mt-5 flex flex-col gap-3 sm:flex-row">
                    <input
                      ...
                      placeholder="Cari NIP, email, atau nama Staff"
                      className="sq-admin-input flex-1"
                    />
                    <button
                      type="button"
                      disabled={staffSearching || Boolean(previewApplications)}
                      onClick={() => void searchStaff()}
                      className="inline-flex items-center justify-center gap-2 rounded-2xl bg-brand-primary px-5 py-3 text-sm font-bold text-white disabled:opacity-50"
                    >
                      {staffSearching ? (
                    ...
                    <div className="space-y-3">
                      {staffResults.map((staff) => (
                        <StaffResult
                          key={staff.subject}
                          staff={staff}
                          selected={selectedStaff?.subject === staff.subject}
                          onClick={() => void selectStaff(staff)}
                        />
                      ))}
                    </div>
                    <div className="rounded-3xl border border-border/70 bg-white p-5 shadow-[var(--shadow-soft)]">
                      {!selectedStaff ? (
                        <EmptyState
                          icon={UserRound}
                          title="Pilih Staff"
                          body="Hasil pencarian akan menampilkan identitas manusia tanpa menampilkan subject OIDC mentah."
                        />
                      ) : (
                        <>
                          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                            ...
                            <div className="flex flex-wrap gap-2">
                              <SecurityPill ok={selectedStaff.security.totpConfigured} label="TOTP" />
                              <SecurityPill
                                ok={selectedStaff.security.recoveryCodesConfigured}
                                label="Kode pemulihan"
                              />
                            </div>
                          </div>
                          <div className="mt-5">
                            <Field label="Alasan perubahan akses">
                              <input
                                value={accessReason}
                                onChange={(event) => setAccessReason(event.target.value)}
                                placeholder="Wajib untuk grant/revoke"
                                className="sq-admin-input"
                              />
                            </Field>
                          </div>
                          {accessLoading ? (
                            <div className="mt-5">
                              <LoadingCard label="Memuat status akses..." compact />
                            </div>
                          ) : (
                                  ...
                                  <div>
                                    <div className="flex items-center gap-2">
                                      ...
                                      <StatusPill
                                        ...
                                        activeLabel="Akses aktif"
                                        inactiveLabel={
                                          item.status === "revoked" ? "Dicabut" : "Belum diberi"
                                        }
                                      />
                                    </div>
                                    <p className="mt-1 text-xs text-muted-foreground">
                                      {item.reason ?? "Belum ada alasan perubahan akses."}
                                    </p>
                                  </div>
                                  <div className="flex gap-2">
                                    <button
                                      type="button"
                                      disabled={item.status === "active" || !accessReason.trim()}
                                      onClick={() => void mutateAccess(item.application.key, "grant")}
                                      className="rounded-xl bg-brand-primary px-3 py-2 text-xs font-bold text-white disabled:opacity-40"
                                    >
                                      Berikan akses
                                    </button>
                                    <button
                                      type="button"
                                      disabled={item.status !== "active" || !accessReason.trim()}
                                      onClick={() => void mutateAccess(item.application.key, "revoke")}
                                      className="rounded-xl border border-destructive/25 bg-destructive/5 px-3 py-2 text-xs font-bold text-destructive disabled:opacity-40"
                                    >
                                      Cabut
                                    </button>
                                  </div>
              ...
              {section === "audit" ? (
                <section className="mt-6">
                  <SectionHeading
                    ...
                    description="Cari perubahan registry, Application Access, dan administrasi platform. Subject OIDC mentah dan material credential tidak ditampilkan."
                  />
                  <div className="mt-5 flex flex-col gap-3 sm:flex-row">
                    <input
                      value={auditQuery}
                      ...
                      placeholder="Cari action, aplikasi, alasan, outcome..."
                      className="sq-admin-input flex-1"
                    />
                    <button
                      type="button"
                      disabled={auditLoading || Boolean(previewApplications)}
                      onClick={() => void loadAudit()}
                      className="inline-flex items-center justify-center gap-2 rounded-2xl border border-border bg-white px-5 py-3 text-sm font-bold text-brand-primary-deep"
                    >
                      {auditLoading ? (
                        <RefreshCw className="h-4 w-4 animate-spin" />
                      ) : (
                  ...
                  <div className="mt-5 overflow-hidden rounded-3xl border border-border/70 bg-white shadow-[var(--shadow-soft)]">
                        ...
                        <tbody className="divide-y divide-border/60">
                          {audit.map((record) => (
                            <tr key={record.id}>
                              ...
                              <td className="px-4 py-3 font-mono text-xs">{record.action}</td>
                              <td className="px-4 py-3 text-xs">{record.targetType}</td>
                              <td className="px-4 py-3">
                                <StatusPill
                                  active={record.outcome === "succeeded"}
                                  activeLabel="Berhasil"
                                  inactiveLabel={record.outcome}
                                />
                              </td>
                              <td className="max-w-sm px-4 py-3 text-xs text-muted-foreground">
                                {auditDescription(record)}
                              </td>
                            </tr>
                          ))}
                        </tbody>
                    ...
                    {audit.length === 0 && !auditLoading ? (
                      <div className="p-7">
                        <EmptyState
                          ...
                          body="Gunakan pencarian untuk memfilter jejak administrasi terbaru."
                        />
                      </div>
                    ) : null}
                  </div>
...
function SectionButton({
  ...
  return (
    <button
      ...
      className={
        active
          ? "inline-flex items-center gap-2 rounded-2xl bg-brand-primary-pale px-4 py-2.5 text-sm font-bold text-brand-primary-deep ring-1 ring-brand-primary/10"
          : "inline-flex items-center gap-2 rounded-2xl border border-border/70 bg-white px-4 py-2.5 text-sm font-semibold text-muted-foreground"
      }
    >
...
function SectionHeading({
  ...
  return (
    <div>
      ...
      <h2 className="mt-1 font-display text-2xl font-bold tracking-[-0.025em] text-brand-heading sm:text-3xl">
        {title}
      </h2>
...
function StaffResult({
  ...
  return (
    <button
      ...
      className={
        selected
          ? "w-full rounded-2xl border border-brand-primary/40 bg-brand-primary-pale/55 p-4 text-left"
          : "w-full rounded-2xl border border-border/70 bg-white p-4 text-left shadow-[var(--shadow-soft)]"
      }
    >
      <div className="flex items-start gap-3">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-brand-primary-pale text-brand-primary-deep">
          <UserRound className="h-5 w-5" />
        </div>
        <div className="min-w-0">
          <p className="truncate font-bold text-brand-heading">{staff.displayName}</p>
          <p className="mt-1 truncate text-xs text-muted-foreground">
            {staff.username}
            {staff.email ? ` · ${staff.email}` : ""}
          </p>
          <div className="mt-2 flex gap-1.5">
            <SecurityPill ok={staff.security.totpConfigured} label="TOTP" />
            <SecurityPill ok={staff.security.recoveryCodesConfigured} label="Recovery" />
          </div>
        </div>
      </div>
    </button>
...
function SecurityPill({ ok, label }: { ok: boolean; label: string }) {
  return (
    <span
      className={
        ok
          ? "inline-flex items-center gap-1 rounded-full bg-brand-primary-pale px-2 py-1 text-[10px] font-bold text-brand-primary-deep"
          : "inline-flex items-center gap-1 rounded-full bg-amber-50 px-2 py-1 text-[10px] font-bold text-amber-700"
      }
    >
      {ok ? <CheckCircle2 className="h-3 w-3" /> : <ShieldAlert className="h-3 w-3" />}
      {label}
    </span>
  );
}
...
function StatusPill({
  ...
  return (
    <span
      className={
        active
          ? "inline-flex rounded-full bg-brand-primary-pale px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-brand-primary-deep"
          : "inline-flex rounded-full bg-muted px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-muted-foreground"
      }
    >
...
function Field({ label, children }: { label: string; children: ReactNode }) {
  return (
    <label className="block">
      <span className="mb-1.5 block text-xs font-bold text-brand-heading">{label}</span>
      {children}
    </label>
  );
}
...
function InlineMessage({ children }: { children: ReactNode }) {
  return (
    <p className="mt-4 rounded-2xl bg-muted/55 px-3.5 py-2.5 text-xs leading-5 text-muted-foreground">
      {children}
    </p>
  );
}
...
function LoadingCard({ label, compact = false }: { label: string; compact?: boolean }) {
  return (
    <section
      className={
        compact
          ? "rounded-2xl border border-border/70 bg-white p-5 text-center"
          : "mt-6 rounded-3xl border border-border/70 bg-white p-8 text-center shadow-[var(--shadow-soft)]"
      }
    >
      <RefreshCw
        className="mx-auto h-5 w-5 animate-spin text-brand-primary-deep"
        aria-hidden="true"
      />
...
function EmptyState({
  ...
  return (
    <div className="py-6 text-center">
      <Icon className="mx-auto h-6 w-6 text-brand-primary-deep" />
      <p className="mt-3 font-bold text-brand-heading">{title}</p>
...
function auditDescription(record: AdminAuditRecord) {
  const applicationKey =
    typeof record.payload.applicationKey === "string" ? record.payload.applicationKey : null;
  const reason = typeof record.payload.reason === "string" ? record.payload.reason : null;
  return (
    [applicationKey, reason].filter(Boolean).join(" · ") ||
    "Tidak ada payload sensitif yang ditampilkan."
  );
}
    ...
    <section className="mt-6 rounded-3xl border border-border/70 bg-white p-7 text-center shadow-[var(--shadow-soft)] sm:p-9">
      ...
      {action ? (
        <button
          ...
          className="mt-5 rounded-2xl bg-brand-primary px-4 py-2.5 text-sm font-bold text-white shadow-[var(--shadow-button)]"
        >
          {action.label}
        </button>
      ) : (
        <a
          href="/"
          className="mt-5 inline-flex rounded-2xl border border-border px-4 py-2.5 text-sm font-bold text-brand-primary-deep"
        >
          Kembali ke SQ Hub
        </a>
      )}
