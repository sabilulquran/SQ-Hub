...
| PostgreSQL-backed Application Access integration suite | Registry, grant/revoke, exact issuer+subject, idempotence, and audit behavior run against PostgreSQL. | Not run: the suite correctly refused to start without `DATABASE_URL`; Docker is unavailable on this workstation. | `npm run test` output: 8 unit tests passed; `application-access.integration.test.ts` stopped before tests with `DATABASE_URL is required for integration tests`. | NEEDS LOCAL/CI DATABASE ENVIRONMENT |
...
**DO NOT ACCEPT Foundation v1 staging UAT yet.**
...
The local SQ Hub API contract tests reduce implementation risk but do not substitute for these authenticated HCIS staging scenarios.

The PostgreSQL-backed integration suite also remains to be run in a configured local or CI database environment; it is a merge-quality evidence gap, not a substitute for the live HCIS scenarios above.
